...
import { TransactionType, Category, Frequency, RecurringTransaction, Account, AppSettings, ExpenseBucket } from '../types';
...
interface AddRecurringTransactionModalProps {
  ...
  categories: Category[];
  onClose: () => void;
  onSave: (transactions: Omit<RecurringTransaction, 'id'>[]) => void;
  transactionToEdit?: RecurringTransaction;
  settings: AppSettings;
  allTags: string[];
  addTag: (tag: string) => void;
  // FIX: Add optional defaultStartDate prop
...
const AddRecurringTransactionModal: React.FC<AddRecurringTransactionModalProps> = ({ accounts, categories, onClose, onSave, transactionToEdit, settings, allTags, addTag, defaultStartDate }) => {
  const [type, setType] = useState<TransactionType>(transactionToEdit?.type || TransactionType.EXPENSE);
  const [accountId, setAccountId] = useState<string>(transactionToEdit?.accountId || accounts[0]?.id || '');
  const [frequency, setFrequency] = useState<Frequency>(transactionToEdit?.frequency || Frequency.MONTHLY);
  // FIX: Use defaultStartDate if provided for new transactions
  const [startDate, setStartDate] = useState(transactionToEdit ? new Date(transactionToEdit.startDate).toISOString().split('T')[0] : defaultStartDate || new Date().toISOString().split('T')[0]);
  const [totalAmount, setTotalAmount] = useState(transactionToEdit?.amount.toString() || '');
  ...
  const firstCategoryId = useMemo(() => categories.find(c => c.type === type)?.id || '', [categories, type]);

  const [splits, setSplits] = useState<Split[]>(transactionToEdit ? [{
        description: transactionToEdit.description,
        amount: transactionToEdit.amount.toString(),
        categoryId: transactionToEdit.categoryId,
        expenseBucket: transactionToEdit.expenseBucket,
  }] : [{
    description: '',
    amount: '',
  ...
  }, [type, categories, categoryMap, transactionToEdit]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
     if (Math.abs(remainingAmount) > 0.001 || !accountId || (parseFloat(totalAmount) || 0) <= 0 || !startDate) {
    ...
    const transactions: Omit<RecurringTransaction, 'id'>[] = splits
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-8 w-full max-w-2xl m-4 max-h-[90vh] flex flex-col">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-gray-800 dark:text-white">{transactionToEdit ? 'Edit' : 'Add'} Recurring Transaction</h2>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200"><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4 flex-grow overflow-y-auto pr-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label htmlFor="account" className="block text-sm font-medium">Account</label>
                    <select id="account" value={accountId} onChange={e => setAccountId(e.target.value)} className="w-full p-2 border rounded-md dark:bg-gray-700 dark:border-gray-600" required>
                ...
                <div>
                    ...
                    <div className="relative">
                        ...
                        <input type="number" id="totalAmount" value={totalAmount} onChange={e => setTotalAmount(e.target.value)} className="w-full pl-7 p-2 border rounded-md dark:bg-gray-700 dark:border-gray-600" required min="0.01" step="0.01"/>
                    </div>
                </div>
                <div>
                    <label htmlFor="frequency" className="block text-sm font-medium">Frequency</label>
                    <select id="frequency" value={frequency} onChange={e => setFrequency(e.target.value as Frequency)} className="w-full p-2 border rounded-md dark:bg-gray-700 dark:border-gray-600">
                ...
                <div>
                    ...
                    <input type="date" id="startDate" value={startDate} onChange={e => setStartDate(e.target.value)} className="w-full p-2 border rounded-md dark:bg-gray-700 dark:border-gray-600" required />
                </div>
            </div>
                ...
                {splits.map((split, index) => (
                    <div key={index} className="p-4 rounded-lg border dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 space-y-3">
                        <div className="flex justify-between items-center">
                            ...
                            {!transactionToEdit && splits.length > 1 && <button type="button" onClick={() => handleRemoveSplit(index)} className="text-red-500 hover:text-red-700 text-sm font-semibold">Remove</button>}
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                           <div>
                                <label className="block text-sm font-medium">Description</label>
                                <input type="text" value={split.description} onChange={e => handleSplitChange(index, 'description', e.target.value)} className="w-full p-2 border rounded-md dark:bg-gray-700 dark:border-gray-600" required/>
                            </div>
                            <div>
                             ...
                             <div>
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                    {Object.values(ExpenseBucket).map(bucket => (
                                        <button type="button" key={bucket} onClick={() => handleSplitChange(index, 'expenseBucket', bucket)} className={`p-2 rounded-lg text-xs font-semibold ${split.expenseBucket === bucket ? 'bg-primary-600 text-white' : 'bg-gray-200 dark:bg-gray-600'}`}>{bucket}</button>
                                    ))}
                                </div>
                             </div>
                         )}
            ...
            {!transactionToEdit && <button type="button" onClick={handleAddSplit} className="w-full py-2 border-2 border-dashed rounded-lg text-sm font-semibold text-primary-600 border-primary-500 hover:bg-primary-50 dark:hover:bg-primary-900/50">+ Add Split</button>}
            
            <div className={`p-3 rounded-lg text-center font-semibold ${Math.abs(remainingAmount) < 0.001 ? 'bg-green-100 dark:bg-green-900 text-green-800' : 'bg-yellow-100 dark:bg-yellow-900 text-yellow-800'}`}>
            ...
            <div className="flex justify-end space-x-4 pt-4 sticky bottom-0 bg-white dark:bg-gray-800 py-2">
                <button type="button" onClick={onClose} className="px-4 py-2 rounded-md bg-gray-200 dark:bg-gray-600 font-semibold">Cancel</button>
                <button type="submit" className="px-6 py-2 rounded-md bg-primary-600 text-white font-semibold hover:bg-primary-700 disabled:bg-primary-400" disabled={Math.abs(remainingAmount) > 0.001 || !totalAmount}>Save</button>
            </div>
        </form>
      </div>
    </div>
